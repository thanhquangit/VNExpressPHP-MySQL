<?php
    $id = $_GET['id'];
    $chitiettin = ChiTietTin($id);
    $row_tin = mysqli_fetch_array($chitiettin);
    CapNhatSoLanXemTin($id);
?>
<h1 class="title"><?php echo $row_tin['title'] ?></h1>

<div class="chitiet">
<!--noi dung-->
  <?php echo $row_tin['content'] ?>
<p class="right"><strong><?php echo $row_tin['name'] ?></strong></p>
<!--//noi dung-->
</div>
<div class="clear"></div>
<a class="btn_quantam" id="vne-like-anchor-1000000-3023795" href="#" total-like="21"></a>
<div class="number_count"><span id="like-total-1000000-3023795"><?php echo $row_tin['qtyView'] ?></span></div>
<!--face-->
...
<div class="clear"></div>
	<ul>
    	<?php
        $tincungloai  = TinCungLoai($row_tin['type_id']);
        while($row_tincungloai = mysqli_fetch_array($tincungloai)){
      ?>
        <li>       
          <a href="#"><img src="upload/tintuc/<?php echo $row_tincungloai['urlImage']?>" alt="<?php echo $row_tincungloai['title'] ?>"></a> <br />
          <a class="title" href="index.php?p=chitiettin&id=<?php echo $row_tincungloai['id'] ?>"><?php echo $row_tincungloai['title'] ?></a>
             <span class="no_wrap">   
        </li>
      <?php    
        }
